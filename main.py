...
# 추첨은 토요일 저녁 8시 35분. 결과가 올라오는 데 시간이 걸리므로
# 넉넉히 21시 30분 이후부터 '추첨 완료' 로 본다.
DRAW_HOUR, DRAW_MIN = 21, 30
...
def is_drawn(n):
    ...
    if d > now.date():
        return False
    return (now.hour, now.minute) >= (DRAW_HOUR, DRAW_MIN)
...
def cached_latest_round():
    ...
    """
    return _LATEST["n"] or latest_drawn_round()
...
def latest_drawn_round():
    """추첨이 끝난 가장 최근 회차."""
    n = latest_round_by_date()
    while n > 1 and not is_drawn(n):
        n -= 1
    return n
...
class DrawStore:
    ...
    def load(self):
        if os.path.exists(DRAWS_CACHE):
            try:
                ...
                print("[!] 캐시 읽기 실패:", e)
        # 앱에 동봉한 과거 당첨번호(1~1171회)를 먼저 깐다.
        # 서버에서 수백 회를 받느라 15분씩 기다릴 일이 없어진다.
        base = []
        try:
            f = asset("seed_draws.json")
            if f:
                with open(f, "r", encoding="utf-8") as fp:
                    base = json.load(fp)
        except Exception as e:
            print("[!] 초기 데이터 읽기 실패:", e)
        if not base:
            base = [{
                "round": r[0],
    ...
    def drop_future(self):
        ...
        self.rows = [r for r in self.rows if is_drawn(int(r["round"]))]
...
_VAL = {"key": None, "val": None, "busy": False}


def validate_cached(rows, on_done=None):
    """
    Popularity.validate 는 순열검정 1500회라 폰에서 2~3초가 걸린다.
    화면을 그릴 때마다 돌리면 탭 전환이 그만큼 늦어지므로,
    결과를 캐시하고 없을 때만 뒤에서 계산한다.
    """
    key = (len(rows), sum(1 for r in rows if r.get("winners")))
    if _VAL["key"] == key:
        return _VAL["val"]
    if _VAL["busy"]:
        return None
    ...
    def work():
        try:
            v = Popularity.validate(rows)
        except Exception as e:
            log_crash(e)
            v = None
        _VAL["key"], _VAL["val"] = key, v
        _VAL["busy"] = False
        if on_done:
            Clock.schedule_once(lambda _dt: on_done(), 0)
...
class Updater:
    ...
    def run(self, rounds, on_progress, on_done):
        ...
        def work():
            ...
            def quick(rnd):
                """1차는 빠른 JSON API 만 쓴다. 실패분은 2차에서 처리."""
                try:
                    if not is_drawn(rnd):
                        return None, "아직 추첨 전"
                    r = fetch_dhlottery(rnd)
                    if r and len(r["nums"]) == 6:
                        return normalize_prize(r), None
                except Exception as e:
            ...
            with ThreadPoolExecutor(max_workers=10) as ex:
                futs = {ex.submit(quick, r): r for r in rounds}
...
class StatScreen(Screen):
    ...
    def _paint_validate(self, *a):
        """백그라운드 검정이 끝나면 그 부분만 다시 그린다."""
        v = _VAL["val"]
        if not v:
    ...
    def refresh(self):
        ...
        # 인기도 모델 검정
        v = validate_cached(DRAWS.rows, on_done=self._paint_validate)
        if v is None and _VAL["busy"]:
            self.pop_a.text = "계산 중…"
            self.pop_b.text = "순열검정을 돌리는 중입니다. 잠시 뒤 표시됩니다."
        elif not v:
            self.pop_a.text = "표본 부족"
            self.pop_b.text = ("당첨자 수가 있는 회차가 15회 이상 필요합니다.\n"
                               "[최신 회차 받기]로 채워집니다.")
...
class MapScreen(Screen):
    ...
    def _download(self, *a):
        ...
        if getattr(self, "_dl", False):
            return
        try:
            return self._download_inner()
        except Exception as e:
            log_crash(e)
            self._unlock_dl()
            toast("자료 받기를 시작하지 못했습니다.")

    def _download_inner(self):
        self._dl = True
        self.dl_btn.text = "…"
        self.dl_btn.disabled = True

        # DRAWS 에는 이 이름의 메서드가 없다. 모듈 함수를 써야 한다.
        latest = latest_drawn_round()
...
class HeungboApp(App):
    ...
    def build(self):
        ...
        def warm():
            # 분산도 기준 분포(2500개)와 순열검정을 미리 돌려 둔다.
            # 통계 탭을 처음 열 때 멈추는 것을 막는다.
            try:
                Popularity.baseline()
                validate_cached(DRAWS.rows)
            except Exception as e:
                log_crash(e)
    ...
    def _auto_sync(self, *a):
        ...
        import time as _t
        now = _t.time()
        need_draw = now - float(PREFS.get("auto_sync_at") or 0) >= 21600
        need_store = now - float(PREFS.get("store_sync_at") or 0) >= 604800

        def work():
            try:
                if need_store:
                    # 판매점 자료가 없거나 일주일이 지났으면 조용히 받아 둔다.
                    try:
                        refresh_stores(
                            list(range(max(1, latest_drawn_round() - 29),
                                       latest_drawn_round() + 1)))
                        PREFS["store_sync_at"] = now
                        save_prefs(PREFS)
                    except Exception as e:
                        log_crash(e)
                if not need_draw:
                ...
                # 동봉 자료에는 당첨자 수가 없다. 매 실행마다 조금씩 채운다.
                need = [r["round"] for r in DRAWS.rows[-300:]
                        if not r.get("winners")][-12:]
                miss = sorted(set(list(miss) + need), reverse=True)[:20]
                if not miss:
                    return
                got = []
                for r in miss:
                    ...
                    for key in ("stat", "my", "map"):
                        ...
                        if sc is not None and hasattr(sc, "on_pre_enter"):
                            try:
                                ...
                                pass
                    toast("최신 자료를 받아 반영했습니다.")
                Clock.schedule_once(fin, 0)
            except Exception as e:
                log_crash(e)

        if need_draw or need_store:
            threading.Thread(target=work, daemon=True).start()
